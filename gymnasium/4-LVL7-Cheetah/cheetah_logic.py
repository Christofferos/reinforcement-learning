    ...
    * v0: Initial versions release.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        xml_path = os.path.join(os.path.dirname(__file__), "cheetah.xml")
        ...
        self.prev_front_contact = False
        self.prev_back_contact = False

        self.last_milestone_x = 0.0  # reset on every episode
        self.milestone_interval = 1.0
    ...
    def n_feet_contact(self):
        front_foot_contact = False
        back_foot_contact = False

        for i in range(self.data.ncon):
            contact = self.data.contact[i]
            geom2 = contact.geom2
            if geom2 == 7:
                back_foot_contact = True
            if geom2 == 10:
                front_foot_contact = True

        if front_foot_contact and not self.prev_front_contact:
            front_step_reward = 1
            self.prev_front_contact = True
            self.prev_back_contact = False
        else:
            front_step_reward = 0

        if back_foot_contact and not self.prev_back_contact:
            back_step_reward = 1
            self.prev_back_contact = True
            self.prev_front_contact = False
        else:
            back_step_reward = 0

        return np.clip(front_step_reward + back_step_reward, 0, 1)
    ...
    def step(self, action):
        ...
        if x_position_after - self.last_milestone_x >= self.milestone_interval:
        ...
        torso_angle = self.data.qpos[2]
        torso_stable_reward = np.clip(0.5 - abs(torso_angle), -1, 1) 
        
        speed_to_energy_reward, reward_info = self._get_rew(x_velocity, action)
        info = {"x_position": x_position_after, "x_velocity": x_velocity, **reward_info}

        joint_positions = self.data.qpos[3:]
        gate_symmetry_reward = np.clip(abs(joint_positions[0] - joint_positions[3]), 0, 1)
        
        alternate_gait_reward = self.n_feet_contact()

        """ print(f"Torso Stable Reward: {torso_stable_reward:.2f}, "
              f"Gate Symmetry Reward: {gate_symmetry_reward:.2f}"
              f", Alternate Gait Reward: {alternate_gait_reward:.2f}"
              f"reward = {reward:.2f}, ") """
        
        step_reward = (
            speed_to_energy_reward
            + milestone_reward
            + 0.8 * torso_stable_reward
            + 0.2 * alternate_gait_reward
            + 0.2 * gate_symmetry_reward
        )

        truncation = False
        self.truncation_timer += 1
        if self.truncation_timer >= (1200 / self.frame_skip):
            truncation = True
            self.truncation_timer = 0
        ...
        return observation, step_reward, False, truncation, info
