...
class HumanoidWalkEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release
    ...
    def step(self, action):
        ...
        foot_separation_x_reward = np.clip(abs(left_foot_x - right_foot_x), 0, 0.2)
        foot_separation_y_reward = 0.1 if abs(left_foot_y - right_foot_y) < 0.5 else 0.0
        feet_lift_reward = 0.0
        if left_foot_z > 0.1 or right_foot_z > 0.1:
            feet_lift_reward = np.clip(np.max([left_foot_z, right_foot_z]) * 2, 0.2, 0.5)
        ...
        if not self.is_healthy:
            fall_penalty = -1000.0 / self.truncation_timer

        torso_z = self.data.qpos[2]
        torso_height_reward = torso_z / 2.0
        ...
        # print("FEET SLIDING PENALTY:", self.sliding_penalty(left_foot_z, right_foot_z))
        sliding_penalty = self.sliding_penalty(left_foot_z, right_foot_z)
        upright_reward = self._reward_upright()
        general_reward = self._get_rew(x_velocity, action, torso_z)

        print("RESULTS: ",  
              "A:", off_track_penalty, 
              "B:", sliding_penalty,
              "C:", center_of_mass_offset_penalty, 
              "D:", low_height_penalty,
              "E:", upright_reward,
              "F:", torso_height_reward,
              "G:", general_reward,
              "H:", feet_lift_reward,
              "I:", bending_knee_reward,
              "J:", milestone_reward,
              "K:", foot_separation_x_reward,
              "L:", foot_separation_y_reward,
              "M:", fall_penalty)
        ...
        step_reward = (
            ...
            bending_knee_reward +
            milestone_reward +
            foot_separation_x_reward +
            foot_separation_y_reward +
            fall_penalty
        )
        ...
        if self.truncation_timer >= (2000 / self.frame_skip): # 800
            truncation = True
