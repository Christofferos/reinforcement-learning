    ...
    * v0: Initial versions release
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.possible_speeds = [2.0, 2.25, 2.5, 2.75, 3.0, 3.25, 3.5] # 0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0, 2.5
        ...
        y_start = np.random.choice([-2, -3, 3, 2])
        x_start = np.random.choice([-4, -3, 4, 3])
    ...
    def _generate_new_target(self):
        ...
        offset = np.array([radius * np.cos(0), radius * np.sin(0)])
        
        # Set the new target point
        current_agent_xy = self.data.qpos[0:2]
        self.target_point_xy = current_agent_xy + [0, 0 - current_agent_xy[1]] + offset # Hardcode: [-2, 0]
    ...
    def sliding_penalty(self, left_foot_z, right_foot_z):
        ...
        return np.clip(sliding_penalty_scale * total_sliding_speed, -0.75, 0.0)
    ...
    def track_velocity_reward(self, x_velocity: float):
        velocity_reward_tolerance = 2.5 # Controls how tolerant the reward is. Higher value = stricter requirement.
        speed_error_sq = (x_velocity - self.target_velocity)**2
        forward_reward_weight = 0.85
    ...
    def step(self, action):
        ...
        if abs(y_position_after) <= 1:
            normalized = 2 * max(0, 0.75 - y_position_after)
        ...
        if left_foot_z > 0.1 or right_foot_z > 0.1 and torso_z > 0.9:
            feet_lift_reward = np.clip(np.max([left_foot_z, right_foot_z]) * 0.35, 0.0, 0.4)
        ...
        # healthy_reward = 1 if self.is_healthy else 0
        ...
        if self.truncation_timer >= (5000 / self.frame_skip): # TODO: DEMO 4000 TRAINING 1500
